...
/**
 * Web: masked duration + portal autocomplete (`DurationPickerDesktop` model).
 */
export function DurationComboField({
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const masked = applyDurationHhmmMask(e.target.value);
      onChange(masked);
      setHighlightIndex(0);
      userNavigatedRef.current = false;
  ...
  const handleKeyDown = useCallback(
    ...
    [allowNone, filteredOptions, highlightIndex, pickPreset, value, onChange]
  );
        ...
        createPortal(
          ...
          >
            {filteredOptions.map((option, i) => {
              ...
              return (
                <div
                  key={option.value === "" ? "none-option" : option.value}
                  role="option"
