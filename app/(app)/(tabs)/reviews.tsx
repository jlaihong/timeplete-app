      ...
      <View style={styles.freqTabs}>
        {(["DAILY", "WEEKLY", "MONTHLY", "YEARLY"] as Frequency[]).map(
          (f) => (
            <TouchableOpacity
              key={f}
              style={[styles.freqTab, frequency === f && styles.activeFreqTab]}
              onPress={() => {
                if (isDirty) {
                  Alert.alert(
                    "Unsaved Changes",
                    ...
                    [
                      { text: "Discard", onPress: () => { setFrequency(f); setOffset(0); } },
                      { text: "Save", onPress: async () => { await handleSave(); setFrequency(f); setOffset(0); } },
                    ]
                  );
                } else {
                  setFrequency(f);
...
const styles = StyleSheet.create({
  ...
  freqTabText: { fontSize: 13, fontWeight: "600", color: Colors.textSecondary },
  activeFreqTabText: { color: Colors.white },
  navigator: {
    flexDirection: "row",
    alignItems: "center",
